<?php
    ...
    $query = "SELECT FECHA_HORA, T, HR, HRDOS, HRA FROM TBNodo2 t1 WHERE t1.HRA <> (SELECT t2.HRA FROM TBNodo2 t2 WHERE t2.id < t1.id ORDER BY t2.id DESC LIMIT 1) AND DATE(FECHA_HORA) = CURDATE() OR t1.id = (SELECT MIN(id) FROM TBNodo2) AND DATE(FECHA_HORA) = CURDATE() ORDER BY FECHA_HORA DESC";
    if($result = $conn->query($query)){
        ...
        if($result->num_rows > 0){
            ...
            $dataRH = array();
            $dataH = array();
            $dataRHNumerica = array();
            $dataRHDosNumerica = array();
